...
After the InsightSynthesisAgent produces its output, this module runs a
self-scoring LLM call that evaluates the synthesis on four dimensions:
  ...
  3. Plain language — is the output free of statistical jargon?
  4. Evidence grounding — are claims linked to observed data patterns?

If the score is below SYNTHESIS_QUALITY_THRESHOLD (default 0.7), the caller
should re-run InsightSynthesisAgent once before proceeding.
"""
...
1. Completeness (0–0.25): Are key_findings, risks_and_bias_signals, and \
recommended_next_analyses all non-empty?
2. Clinical relevance (0–0.25): Are the findings and recommendations relevant \
to the stated study context?
3. Plain language (0–0.25): Is the output free of raw statistical values, \
p-values, and engineering jargon?
4. Evidence grounding (0–0.25): Are claims linked to observable data patterns \
...
def score_synthesis_quality(
    ...
    """
    Score the synthesis output and return a SynthesisQualityScore.
