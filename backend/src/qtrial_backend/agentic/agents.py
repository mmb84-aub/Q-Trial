...
_DQ_USER = textwrap.dedent("""\
    ...
    EVIDENCE POINTER RULES (mandatory — violations will cause rejection):
      ...
      Do NOT invent paths. Do NOT use keys absent from the evidence JSON.
...
_IS_SYSTEM = textwrap.dedent("""\
    ...
    - Do NOT invent citation paths not present in RESOLVABLE_CITATIONS.
...
_IS_USER = textwrap.dedent("""\
    ...
    Required JSON schema:
    {{
      "key_findings": ["<finding with inline citation from RESOLVABLE_CITATIONS>", ...],
      ...
      "recommended_next_analyses": [
        {{
          ...
          "evidence_citation": "<verbatim string(s) from RESOLVABLE_CITATIONS>"
        }}
      ],
